...
const total = function (page, Filter, ITEMS, CAT) {
  ...
  if (page == null || page == '') {
    ...
    { return (ITEMS.filter(item => item.category == CAT.filter(cat => (cat.name === page)).map(cat => (cat.id)))).length }
  }
}
...
const available = function (page, Filter, ITEMS, CAT) {
  if (page == null || page == '') {
    ...
    return ITEMS.filter(item => item.category == Filter && item.available)
      .map((item) =>
        (item)).length
  }
  else {
    {
      return ITEMS.filter(item => item.category == CAT.filter(cat => (cat.name === page)).map(cat => (cat.id)))
